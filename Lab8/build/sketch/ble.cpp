#line 1 "/Users/yangd19/Documents/Code/ECE5160-Code/Lab8/lab8-main/ble.cpp"
#include "main.h"
#include "ble.h"
...
void putARecordToTxEstring(const Record *record, uint16_t index) {
    ...
    tx_estring_value.append("|");
    tx_estring_value.append((int)record->setpoint_distance);
    tx_estring_value.append("|");
    tx_estring_value.append((int)record->yaw);
    tx_estring_value.append("|");
    tx_estring_value.append((int)record->setpoint_yaw);
    tx_estring_value.append("|");
...
void handleCommand() {   
    ...
    switch (cmd_type) {
        ...
        case SET_GOAL_YAW: {
            ...
            success = robot_cmd.get_next_value(num);
            if (!success) {
                tx_estring_value.append("Failed: unable to get goal yaw");
                tx_characteristic_string.writeValue(tx_estring_value.c_str());
                break;
            }
            if (num < -180 || num > 180) {
                tx_estring_value.append("Failed: goal yaw out of range");
                tx_characteristic_string.writeValue(tx_estring_value.c_str());
                break;
            }
            goal_yaw = num;
